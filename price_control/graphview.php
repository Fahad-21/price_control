...
-->			</div>
		...
		<div class="body">
								<b> Changes of Fruit Price in Different Cities</b>
...
<ul class="yAxis">
	...
	<dl id="csschart">
		<?php
		$Index = 0;
		$Query = "SELECT main.price,city.cid,main.date FROM   `main`  INNER JOIN city ON main.cid= city.cid  WHERE fid = '".$_REQUEST['fid']."' and date = '".$_REQUEST['date']."' ORDER BY date ";
...
<ul class="xAxis">
	<?php
		$Index = 0;
		$Query = "SELECT date FROM `main` WHERE fid = '".$_REQUEST['fid']."' and date = '".$_REQUEST['date']."'  ";
		$rstRow = mysql_query($Query);
		while($objRow = mysql_fetch_object($rstRow)){
			$Index++;
