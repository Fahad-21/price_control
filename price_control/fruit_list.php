	...
	if(isset($_REQUEST['Submit'])){
	   ...
	   // echo $Query0; die;
		...
		if($rows0 >=1){
			...
			header("location: date1.php?error");
		}
	}
...
-->			</div>
		...
		<div style="margin-left:152px;background-color:#f6efe5;width:960px;height:auto;margin-top:-55px" >
		<div style="margin-left:20px;width:650px;height:auto;" class="page">	
	<h1 style="color:#006" class="acc">Price Of Goods</h1>
				
<!--					<span><marquee scrollamount="4px" behavior="alternate" >Please select a City to view the according list of goods</marquee></span>
-->					<fieldset style="height:auto" class="ax1">
							<legend ><b>List Of Goods</b></legend>
							 <table cellpadding="0" cellspacing="0" border="1" width="616px" id="mt" class="page">
...
-->		
	...
	<?php
		$Index=0;
		$Query = "SELECT main.price,main.date,city.name FROM `main`  INNER JOIN city ON main.cid= city.cid  WHERE fid = '".$_REQUEST['fid']."' ORDER BY date desc";
	...
	<?php
		...
		$Query2 = "SELECT * FROM fruits  WHERE fid ='".$_REQUEST['fid']."' ";
		//echo $Query; die();
	...
	<tr >
		<td align="center" class="ac1"><?=$Index;?></td>
		<td align="center" class="ac1"><?php echo $objRow2->name;?></td>
		<td align="center" class="ac1"><?php echo $objRow2->unitt;?></td>
		<td style="text-align:center" class="ac1"><?php echo $objRow->price;?></td>
		<td style="text-align:center" class="ac1"><?php echo $objRow->date;?></td>
		<td style="text-align:center" class="ac1"><?php echo $objRow->name;?></td>
	</tr>
	<?php } ?>
	<?php } ?>
	...
	<?php
		$Index=0;
		$Query = "SELECT main1.price,main1.date,city.name FROM `main1`  INNER JOIN city ON main1.cid= city.cid  WHERE vid = '".$_REQUEST['vid']."' ORDER BY date desc";
		...
		$Query3 = "SELECT * FROM vegetables  WHERE vid ='".$_REQUEST['vid']."' ";
		//echo $Query; die();
	...
	<tr>
		...
		<td style="text-align:center"><?php echo $objRow3->name;?></td>
		<td style="text-align:center"><?php echo $objRow3->unit;?></td>
